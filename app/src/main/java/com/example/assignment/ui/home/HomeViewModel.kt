...
import com.example.assignment.Callback.IBestDealLoadCallback
import com.example.assignment.Callback.IPopularLoadCallback
...
class HomeViewModel : ViewModel(), IPopularLoadCallback, IBestDealLoadCallback {
    ...
    private var  popularListMutableLiveData: MutableLiveData<List<PopularGategoryModel>>?=null
    ...
    val popularList:LiveData<List<PopularGategoryModel>>
    get() {
    ...
    private fun loadPopularList() {
       val tempList = ArrayList<PopularGategoryModel>()
        ...
        popularRef.addListenerForSingleValueEvent(object: ValueEventListener{
            ...
            override fun onDataChange(p0: DataSnapshot) {
                for (itemSnapShot in p0!!.children)
                {
                    val model = itemSnapShot.getValue<PopularGategoryModel>(PopularGategoryModel::class.java)
                    tempList.add(model!!)
                }
    ...
    override fun onPopularLoadSuccess(popularModelList: List<PopularGategoryModel>) {
